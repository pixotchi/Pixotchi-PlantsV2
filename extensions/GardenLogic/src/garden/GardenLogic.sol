...
PixotchiExtensionPermission
{
    ...
    function Revive(uint256 _Id) external  {
        require(IGame(address(this)).isPlantAlive(_Id) == false, "your plant is not dead");
        require((IERC721A(address(this)).ownerOf(_Id) == msg.sender), "Invalid Owner");

        _s().guardDisarmed = true;
        INFT(address(this)).tokenBurnAndRedistribute(msg.sender, _s().Revive_Price);
        _s().guardDisarmed = false;

        _s().plantTimeUntilStarving[_Id] = block.timestamp + 24 hours;

        emit Revived(_Id, msg.sender, _s().plantName[_Id]);
    }

    function setRevivePrice(uint256 _Revive_Price) public onlyAdminRole {
        _s().Revive_Price = _Revive_Price;
    }

    function getRevivePrice() public view returns(uint256) {
       return _s().Revive_Price;
    }
