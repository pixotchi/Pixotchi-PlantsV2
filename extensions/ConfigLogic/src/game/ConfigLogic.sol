...
PixotchiExtensionPermission
{
    ...
    function setKillIsActive(bool _KillIsActive) external onlyAdminRole {
        _s().KillIsActive = _KillIsActive;
    }

    function setChangePlantNamePrice(uint256 _ChangePlantName_Price) external onlyAdminRole {
        _s().ChangePlantName_Price = _ChangePlantName_Price;
    }

    function setTOD(uint256 plantId, uint256 tod) external onlyAdminRole {
        _s().plantTimeUntilStarving[plantId] = block.timestamp + tod;
    }
